...
std::unordered_map<AvatarLevelOfDetail, ovrAvatarAssetLevelOfDetail, AvatarLevelOfDetailHash> AVoIPLocalAvatar::LODMap
(
	{
...
AVoIPLocalAvatar::AVoIPLocalAvatar()
{
...
void AVoIPLocalAvatar::PreInitializeComponents()
{
...
void AVoIPLocalAvatar::BeginPlay()
{
	Super::BeginPlay();
	UE_LOG(LogTemp, Warning, TEXT("BEGIN PLAY"));
}

void AVoIPLocalAvatar::BeginDestroy()
{
	Super::BeginDestroy();
}

void AVoIPLocalAvatar::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (UseLocalMicrophone)
	{
		LipSyncComponent->OnVisemesReady.RemoveDynamic(this, &AVoIPLocalAvatar::LipSyncVismesReady);
	}
}

void AVoIPLocalAvatar::Tick(float DeltaTime)
{
...
void AVoIPLocalAvatar::InitializeLocalAvatar(FString OculusUserID)
{
	...
	if (IsUsingAvatars)
	{
		if (UseLocalMicrophone)
		{
			LipSyncComponent->OnVisemesReady.AddDynamic(this, &AVoIPLocalAvatar::LipSyncVismesReady);
...
void AVoIPLocalAvatar::InitializeRemoteAvatar()
{
...
void AVoIPLocalAvatar::UpdatePacketRecording(float DeltaTime)
{
...
void AVoIPLocalAvatar::ServerHandleAvatarPacket_Implementation(const FOculusAvatarPacket& PacketData)
{
...
bool AVoIPLocalAvatar::ServerHandleAvatarPacket_Validate(const FOculusAvatarPacket& PacketData)
{
		return true;
}
...
void AVoIPLocalAvatar::OnRep_PacketData()
{
	bAvatarIsLocal = (Controller && Controller->IsLocalController());
	if ((!bAvatarIsLocal) && AvatarComponent)
	{
...
//	Replication List
void AVoIPLocalAvatar::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	// Replicate to everyone
	DOREPLIFETIME_CONDITION(AVoIPLocalAvatar, ReplicatedPacketData, COND_SkipOwner);
	DOREPLIFETIME(AVoIPLocalAvatar, OculusID);
	DOREPLIFETIME(AVoIPLocalAvatar, IsUsingAvatars);
}

void AVoIPLocalAvatar::SetRightHandTransform(TArray<float> LocalTransforms)
{
...
void AVoIPLocalAvatar::ResetRightHandTransform()
{
	RightHandPoseIndex = eHandPoseState::Default;
	AvatarComponent->SetRightHandPose(ovrAvatarHandGesture_Default);
}

void AVoIPLocalAvatar::SetLeftHandTransform(TArray<float> LocalTransforms)
{
...
void AVoIPLocalAvatar::ResetLeftHandTransform()
{
	LeftHandPoseIndex = eHandPoseState::Default;
	AvatarComponent->SetLeftHandPose(ovrAvatarHandGesture_Default);
}

void AVoIPLocalAvatar::SetLeftHandVisibility(bool LeftHandVisible)
{
	AvatarComponent->SetLeftHandVisible(LeftHandVisible);
}

void AVoIPLocalAvatar::SetRightHandVisibility(bool RightHandVisible)
{
	AvatarComponent->SetRightHandVisible(RightHandVisible);
}

void AVoIPLocalAvatar::SetControllersVisibility(bool ControllersVisible)
{
	AvatarComponent->SetHandsOnControllers(ControllersVisible);
}

void AVoIPLocalAvatar::LipSyncVismesReady()
{
		AvatarComponent->UpdateVisemeValues(LipSyncComponent->GetVisemes(), LipSyncComponent->GetLaughterScore());
}

UOvrAvatar::MaterialType AVoIPLocalAvatar::GetOvrAvatarMaterialFromType(AvatarMaterial material)
{
	switch (material)
	{
...
void AVoIPLocalAvatar::AvatarVisibility()
{
	AvatarComponent->SetVisibilityType(AvatarVisibilityType == AvatarVisibility::FirstPerson ? ovrAvatarVisibilityFlag_FirstPerson : ovrAvatarVisibilityFlag_ThirdPerson);
}