	...
	for (uint32_t CompIndex = 0; CompIndex < ComponentCount; ++CompIndex)
	{
		...
		for (uint32_t RenderIndex = 0; RenderIndex < AvatarComponent->renderPartCount; ++RenderIndex)
		{
			...
			case ovrAvatarRenderPartType_SkinnedMeshRenderPBS_V2:
			{
				...
				FOvrAvatarManager::Get().CacheRoughnessMapID(RenderData->materialState.metallicnessTextureID);
			}
			break;
			default:
				break;
...
void UOvrAvatar::SetLeftHandVisible(bool LeftHandVisibility)
{
	ovrAvatar_SetLeftHandVisibility(Avatar, LeftHandVisibility);
}

void UOvrAvatar::SetRightHandVisible(bool RightHandVisibility)
{
	ovrAvatar_SetRightHandVisibility(Avatar, RightHandVisibility);
}

void UOvrAvatar::SetHandsOnControllers(bool ControllerVisibility)
{
	ovrAvatar_SetLeftControllerVisibility(Avatar, ControllerVisibility);
	ovrAvatar_SetRightControllerVisibility(Avatar, ControllerVisibility);
}