...
bool AccountId::IsValid() const
{
    ...
    auto ConstItrD = _displayName.CreateConstIterator();
    while (_displayName.IsValidIndex(ConstItrD.GetIndex()))
        if (!ValidCharacters.FindChar(_displayName[ConstItrD++.GetIndex()], loc))
            return false;
