...
void ChannelSession::HookUnhookEvents()
{
    ensure(!_detached);
    if(_audioState == ConnectionState::Disconnected && _textState == ConnectionState::Disconnected && _eventSdkEventRaised.IsValid()) {
        CleanupEventHandler();
    } else if(!_eventSdkEventRaised.IsValid() &&
        (_audioState == ConnectionState::Connecting || _audioState == ConnectionState::Connected ||
            _textState == ConnectionState::Connecting || _textState == ConnectionState::Connected)) {
...
VivoxCoreError ChannelSession::BeginSetChannelTranscription(bool value, const FString &accessToken, FOnBeginSetChannelTranscriptionCompletedDelegate theDelegate)
{
    ...
    innerDelegate.BindLambda([this, theDelegate, protect](const vx_resp_base_t &resp)
    {
        ...
        if (resp.return_code == 0) { // if successful, set current state to requested state, otherwise no change
            _isSessionBeingTranscribed = reinterpret_cast<vx_req_session_transcription_control_t *>(resp.request)->enable;
        }
        theDelegate.ExecuteIfBound(resp.status_code);
    });
