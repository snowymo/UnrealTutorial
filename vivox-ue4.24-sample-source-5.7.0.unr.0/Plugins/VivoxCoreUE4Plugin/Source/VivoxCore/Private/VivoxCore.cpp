...
// Workaround for VNS-2359. Remove once released (expected in 5.8.0)
#include <nn/socket.h>
extern "C"
in_addr_t inet_addr(const char *cp)
{
    in_addr addr;
    if (nn::socket::InetAton(cp, &addr)) {
        return addr.s_addr;
    } else {
        return INADDR_NONE;
    }
...
void *FVivoxCoreModule::LoadALibrary(const TCHAR *libraryName)
{
    ...
    FString LibraryPath;
#if PLATFORM_WINDOWS
    LibraryPath = FPaths::Combine(*BaseDir, TEXT("Source/ThirdParty/VivoxCoreLibrary/Windows/x64/Release"));
#elif PLATFORM_MAC
