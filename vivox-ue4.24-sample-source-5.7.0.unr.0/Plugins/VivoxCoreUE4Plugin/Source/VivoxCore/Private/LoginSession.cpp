...
void LoginSession::HandleEvent(const vx_evt_base_t& evt)
{
    if (evt.type == evt_account_login_state_change)
    {
        ...
        if (_state == LoginState::LoggedIn && _loginSessionId.ToString() == FString(tevt.account_handle)) {
            AccountId buddyAccount = AccountId::CreateFromUri(tevt.buddy_uri, FString(tevt.displayname));
...
void LoginSession::HookUnhookEvents()
{
    if (_state == LoginState::LoggedOut && _eventSdkEventRaised.IsValid()) {
        CleanupEventHandler();
    }
    else if (!_eventSdkEventRaised.IsValid() && (_state == LoginState::LoggingIn || _state == LoginState::LoggedIn)) {
