...
AudioDevicesImpl::AudioDevicesImpl(AudioDeviceType type): _noDevice("No Device", "No Device", DeviceType::NullDevice), _systemDevice("Default System Device", "Default System Device", DeviceType::DefaultSystemDevice),
                                                          _communicationDevice("Default Communication Device", "Default Communication Device", DeviceType::DefaultCommunicationDevice), _volumeAdjustment(0), _muted(false)
{
    _type = type;
}
...
{
    if (newerId.IsEmpty() || !newerId.Compare("No Device"))
    {
...
VivoxCoreError AudioDevicesImpl::SetActiveDevice(const IAudioDevice& device, const FOnSetActiveDeviceCompletedDelegate& theDelegate)
{
    ...
    innerDelegate.BindLambda([this, theDelegate](const vx_resp_base_t &resp)
    {
        ...
        {
            ensure(_availableDevices.Contains(deviceId));
            _activeDeviceId = deviceId;

            // If switching a virtual device to active, we must set _effectiveDeviceId to the underlying physical device Id.
            FString physicalId = (deviceId == "Default System Device") ? _systemDevice.Id() : (deviceId == "Default Communication Device") ? _communicationDevice.Id() : deviceId;
            ensure(physicalId.IsEmpty() || _availableDevices.Contains(physicalId));
            if (MaybeChangeDeviceId(_effectiveDeviceId, physicalId))
            {
                if (_effectiveDeviceId.IsEmpty())
                {
                    // The way the plugin is currently set up, *manually* selecting 'No Device' shouldn't be possible; if it still happens, display this warning.
                    UE_LOG(VivoxCore, Warning, TEXT("Effective device is now a virtual 'Null' device. This usually means no physical device is present (or on Xbox One, that no user is signed in). Selecting no audio device is perfectly valid, but you should check your setup if you were expecting one."));
    ...
    FString virtualId = (static_cast<const AudioDeviceImpl&>(device).Type() == DeviceType::DefaultSystemDevice) ? "Default System Device" : 
                 (static_cast<const AudioDeviceImpl&>(device).Type() == DeviceType::DefaultCommunicationDevice) ? "Default Communication Device" : device.Id();
...
const IAudioDevice& AudioDevicesImpl::ActiveDevice()
{
    if (_activeDeviceId.IsEmpty())
        return _noDevice;
    ensure(_availableDevices.Contains(_activeDeviceId));
...
const IAudioDevice& AudioDevicesImpl::EffectiveDevice()
{
    if (_effectiveDeviceId.IsEmpty())
        return _noDevice;
    ensure(_availableDevices.Contains(_effectiveDeviceId));
...
static IAudioDevice *IAudioDeviceFromVxDevice(vx_device_t *device)
{
    ...
    return new AudioDeviceImpl(device->display_name, device->device, type);
}
...
void AudioDevicesImpl::Handle(const vx_resp_aux_get_capture_devices_t &r)
{
    ...
    _systemDevice = AudioDeviceImpl(r.default_capture_device->display_name, r.default_capture_device->device, DeviceType::DefaultSystemDevice);
    _communicationDevice = AudioDeviceImpl(r.default_communication_capture_device->display_name, r.default_communication_capture_device->device, DeviceType::DefaultCommunicationDevice);
...
void AudioDevicesImpl::Handle(const vx_resp_aux_get_render_devices_t &r)
{
    ...
    _systemDevice = AudioDeviceImpl(r.default_render_device->display_name, r.default_render_device->device, DeviceType::DefaultSystemDevice);
    _communicationDevice = AudioDeviceImpl(r.default_communication_render_device->display_name, r.default_communication_render_device->device, DeviceType::DefaultCommunicationDevice);
    if (_systemDevice.IsEmpty())
    {
